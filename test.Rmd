...
```{r}
careerBatting100 <- sample(careerBatting, 25, replace = FALSE)
```

```{r}
dd <- dist(scale(careerBatting100), method = 'euclidean')
dd <- get_dist(careerBatting100, method = "euclidean")
hc_career <- hclust(dd, method = "single")
```
```{r}
plot(dend, labels = career_hc$playerID)
```

```{r}
eu_career <- dist(careerBatting, method = "euclidean", upper = TRUE)
eu_career <- get_dist(careerBatting, method = 'euclidean')
```
```{r}
hc_sing <- hclust(eu_career, method = 'single')
```
...
```{r}
ggplot(data = career_Babe, aes(x = playerID, y = HR)) +
  geom_bar(stat = "identity")
```
=======
>>>>>>> 16166b05fe9293ac70ba6235a6ab52c23e1a708e
